...
from src.utills.main_utils import read_yaml_file,write_yaml_file
...
class DataValidation:
    ...
    def is_column_exist(self,dataframe:pd.DataFrame)->bool:
        try:
            ...
            if not numerical_columns_present:
                logging.info(f"missing_columns [{missing_numerical_columns}]")
            else:
    ...
    def initiate_data_validation(self)->DataIngestionArtifact:
        try:
            ...
            if not status:
                error_message = f"{error_message} train dataframe does not contain all above columns"
            status = self.is_column_exist(dataframe=test_dataframe)
            if not status:
                error_message = f"{error_message} test dataframe does not contain all above columns"
